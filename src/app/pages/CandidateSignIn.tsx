    ...
    try {
      ...
      if (result.token) {
        ...
        if (result.user.userType === 'candidate') {
          ...
          toast.error("Please use the recruiter sign in page");
          clearAuthSession();
        }
      } else {
      ...
      <div className="flex-1 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
        <div className="max-w-md w-full">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-4 sm:p-8">
            ...
            <div className="mt-6">
              <GoogleSignInButton userType="candidate" action="signin" />
            </div>

            <div className="mt-6 text-center">
              <span className="text-sm text-gray-600 dark:text-gray-400">Are you a recruiter? </span>
              <Link to="/recruiter/signin" className="text-sm text-blue-600 hover:text-blue-700">
                Sign in here
              </Link>
            </div>
          </div>
